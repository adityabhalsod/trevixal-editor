...
| `Mod+Shift+X` | Strikethrough | | `Mod+Alt+0` | Normal text |
| `Mod+Alt+1` to `Mod+Alt+6` | Heading 1 to 6 | | `Mod+Shift+7` / `8` / `9` | Numbered / bullet / task list |
...
| `Mod+Alt+E` | Emoji picker | | `Shift+Enter` | Line break |
...
The paragraph keys are Google Docs' rather than Word's: Word's were made for a
desktop app and collide in a browser (`Ctrl+E` is inline code here, `Ctrl+L`
the address bar). A key is matched by the key pressed, so `Mod+Shift+7` works
on a layout where Shift turns 7 into `/`. `Ctrl+Alt` off a Mac is AltGr, which
types characters, so it never matches that way: on a keyboard where AltGr+2
types `²`, it goes on typing it.

*Change* in the dialog wants Ctrl, Alt or ⌘ (or a function key) with the key:
a letter on its own would fire instead of typing. The search box narrows the
list by name or by keys.
...
picker dialog (`Mod+Alt+E` in the assembled editor).
