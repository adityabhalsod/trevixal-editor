...
`@trevixal/extension-slash-command`: the `/` menu. Type a slash at the start
of a line and pick a block.
...
const slash = slashCommand(editor, {
  items: [
    ...
    { id: 'table', title: 'Table', keywords: ['grid', 'rows'], run: (e) => e.exec(insertTable({ rows: 3, cols: 3 })) },
  ],
...
Text, Heading 1 to 3, Bullet list, Numbered list, Code block, Quote, Divider,
...
beats any subsequence. Typing `co` puts "Code block" above "Bullet list",
even though both contain the letters.

**The keyboard is the point.** Arrows move, `Enter` and `Tab` select,
`Escape` dismisses, and picking an item removes the `/` and the query before
running it, so a command never has to clean up after the menu.

Both this menu and the [emoji picker](./emoji) ride on one trigger driver in
the core, `suggestionList`, which you can reuse for `@`-style triggers of your
own.
